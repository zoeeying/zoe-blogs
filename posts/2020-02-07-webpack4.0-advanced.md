...
**步骤2：** package.json 中配置 sideEffects 为 false，表示对所有模块都进行 Tree Shaking。
...
**步骤3：** 如果 mode 为 development，Webpack 打包的时候，只是会提示我们 used 了哪些模块，而不会把没有 used 的模块删掉，如果删掉的话，Source Map 映射的行数就会错了，不便于我们调试代码。如果 mode 为 production，webpack.config.js 中的 optimization 选项就没有必要了，Webpack 打包的时候，会删掉我们没有 used 的模块。
...
**步骤3：** 编写 index.js 代码，对于异步加载的 lodash 打包后会被单独放在一个文件中。
...
**步骤1：** 移除上面的 babel-plugin-dynamic-import-webpack 插件，因为该插件不支持魔法注释，安装官方的动态引入模块的插件 @babel/plugin-syntax-dynamic-import。
...
**步骤4：** 配置 Webpack 配置文件中的 optimization 选项。
...
**步骤1：** 使用命令 `npm init -y` 初始化项目

**步骤2：** 安装 webpack 和 webpack-cli

```bash
cnpm install webpack webpack-cli -S
```

**步骤3：** 新建 Webpack 配置文件 webpack.config.js
...
**步骤4：** 在 package.json 中配置 main。

```json
"main": "./dist/library.js"
```

**步骤5：** 登录网站 www.npmjs.com 注册账号

**步骤6：** 在项目目录下执行命令 `npm adduser`，添加用户名和密码

**步骤7：** 执行命令 `npm publish`，把项目发布到 npm 仓库上去
