...
**步骤4：** 在项目根目录中创建 pages 文件夹，在 pages 文件夹中创建 index.js 文件：
...
**步骤1：** 把 .babelrc 中的 `"style": "css"` 去掉

**步骤2：** 在 pages 目录中新建 _app.js 文件（写所有全局的东西），然后在 _app.js 中全局引入样式文件
...
**步骤3：** 通过命令 `yarn build` 对项目进行打包，通过命令 `yarn start` 开启服务