...
ConfigMap 是一种 API 对象，用于将非加密数据保存到键值对中，使用时，Pods 可以将其用作环境变量、命令行参数或者存储卷中的配置文件。ConfigMap 可以将环境变量配置信息与容器镜像解耦，便于应用配置的修改。

下面使用 Redis 来实战 ConfigMap：
...
创建好配置集后，创建一个 Redis Pod，并引用该配置集：
...
spec:
  ...
  - name: redis
    ...
    command:
      - redis-server
      - "/redis-master/redis.conf" # Redis容器中的位置
    ports:
    - containerPort: 6379
    volumeMounts:
    - mountPath: /data
...
如果修改了 ConfigMap，Pod 里的配置文件会跟着变化，但是需要重新启动 Pod，让 Pod 重新加载配置文件，才能从关联的 ConfigMap 中获取更新后的值，使其生效。
...
如果拉取 Docker Hub 私有镜像，需要输入用户名密码，可以把 Docker Hub 用户名密码创建成密钥 zoe-docker：
